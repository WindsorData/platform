...
import _root_.mapping.Top5MappingComponent
      ...
      model =>
        model.validateWhen(model.contains('executives) && (model /#/ 'disclosureFiscalYear) == lastYear) {
          val results: Seq[Validated[Model]] =
            model.applySeq(Path('executives, *)).map {
              exec =>
      ...
      Valid(models.map {
        model =>
          if (model.contains('executives)) {
            val execsWithCalcs = model.applySeq(Path('executives, *)).map {
              m => {
            ...
            model.merge(Model('executives -> Col(execsWithCalcs.filter(_ /! 'lastName nonEmpty).map {
              exec =>
    ...
    reduceExecutiveValidations(Path('executives, *), model) {
      m => {
        val result: Seq[Validated[Model]] = Seq(
          ...
          m / 'cashCompensations /% 'targetBonus -> m / 'cashCompensations / 'nextFiscalYearData /% 'targetBonus).map {
          it =>
            it match {
