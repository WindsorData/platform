...
class Top5ValidationsSpec extends FunSpec {
  ...
  describe("top 5 workbook validations") {
    ...
    it("should validate calculation for max bonus & threshold bonus") {
      def model(maxBonusValue: Value[BigDecimal], thresholdBonus: Value[BigDecimal]) =
        createModel(
          Model(
            'cashCompensations -> Model(
              'maxBonus -> maxBonusValue,
              'thresholdBonus -> thresholdBonus)
          )
        )
      assert(top5.formulaValidation(model(Value(), Value())).isValid)
      assert(top5.formulaValidation(
          model(Value(Some(100), Some("=50+50"), None, None, None),
            Value(Some(5), Some("=4+1"), None, None, None))).isValid)
      assert(top5.formulaValidation(
          model(Value(Some(100), None, None, None, None),
            Value(Some(2), Some("=4/2"), None, None, None))).isValid)
      assert(top5.formulaValidation(
          model(Value(Some(2), Some("=4/3"), None, None, None),
                Value())).isInvalid)
    }
  }
  ...
  describe("top 5 sheet validations") {
    ...
    it("should validate on time vest rs, if price * number == value") {
      ...
      assert(top5.timeVestRsValueValidation(model(Value(2), Value(3000), Value(4))).isDoubtful)
    }
  }
}
