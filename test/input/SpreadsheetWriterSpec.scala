...
class SpreadsheetWriterSpec extends FunSpec {
  ...
  describe("An Exporter") {
    ...
    it("should export executives from excel into an excel") {
      val companies = FileManager.loadSpreadsheet("test/input/FullValuesOnly.xlsx").toSeq
      FileManager.generateNewFileWith("test/input/outputTest1.xlsx", companies)
    ...
    it("should export executives from model into excel") {
      ...
      FileManager.generateNewFileWith("test/input/outputTest2.xlsx", Seq(company))
      validateFileExistance("outputTest2")
    }
    
    it("should export companies from model into excel") {
      val companies =
        Seq(
        CompanyFiscalYear(
          ticker = Input(Some("ticker"), Some("note ticker"), None),
        ...
        CompanyFiscalYear(
          ticker = Input(Some("ticker2"), Some("note ticker2"), None),
          name = Input(Some("coname2"), Some("note coname2"), None),
          disclosureFiscalYear = Input(Some(2000), None, None),
          originalCurrency = None,
          currencyConversionDate = None,
          executives = executives))
          

      FileManager.generateNewFileWith("test/input/outputTest3.xlsx", companies)
      validateFileExistance("outputTest3")
    }
